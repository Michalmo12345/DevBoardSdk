...
static void spi_transmit_receive(void *self, uint8_t *txData, uint8_t *rxData, size_t size)
{
    Spi *spi = (Spi *)self;
    HAL_GPIO_WritePin((GPIO_TypeDef *)spi->csPort, spi->csPin, GPIO_PIN_RESET);
    HAL_SPI_TransmitReceive((SPI_HandleTypeDef *)self, txData, rxData, size, HAL_MAX_DELAY);
    HAL_GPIO_WritePin((GPIO_TypeDef *)spi->csPort, spi->csPin, GPIO_PIN_SET);
