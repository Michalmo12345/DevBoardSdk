...
static void spi_configure(void *self, SPI_HandleTypeDef *spi_handle,
...
static void spi_transmit(void *self, uint8_t *data, size_t size)
{
    ...
    HAL_SPI_Transmit(self, data, size, HAL_MAX_DELAY);
...
static void spi_receive(void *self, uint8_t *data, size_t size)
{
    ...
    Spi *spi = (Spi *)self;
    HAL_GPIO_WritePin(spi->csPort, spi->csPin, GPIO_PIN_RESET);
    HAL_SPI_Receive(self, data, size, HAL_MAX_DELAY);
    HAL_GPIO_WritePin(spi->csPort, spi->csPin, GPIO_PIN_SET);
}
                                 ...
                                 size_t size)
{
    Spi *spi = (Spi *)self;
    HAL_GPIO_WritePin(spi->csPort, spi->csPin, GPIO_PIN_RESET);
    HAL_SPI_TransmitReceive(self, txData, rxData, size, HAL_MAX_DELAY);
    HAL_GPIO_WritePin(spi->csPort, spi->csPin, GPIO_PIN_SET);
}
