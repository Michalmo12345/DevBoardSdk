...
static void i2c_configure(void *self, void *i2c_handle, uint16_t device_address) {
    I2c *i2c = (I2c *)self;
...
static void i2c_transmit(void *self, uint8_t *data, size_t size) {
    ...
    HAL_I2C_Master_Transmit((I2C_HandleTypeDef *)i2c->i2c_handle, 
                            i2c->device_address, data, size, HAL_MAX_DELAY);
}

static void i2c_receive(void *self, uint8_t *data, size_t size) {
    I2c *i2c = (I2c *)self;
    HAL_I2C_Master_Receive((I2C_HandleTypeDef *)i2c->i2c_handle, 
                           i2c->device_address, data, size, HAL_MAX_DELAY);
}

static void i2c_transmit_receive(void *self, uint8_t *txData, uint8_t *rxData, size_t size) {
    I2c *i2c = (I2c *)self;
    HAL_I2C_Mem_Read((I2C_HandleTypeDef *)i2c->i2c_handle, 
                     i2c->device_address, txData[0], I2C_MEMADD_SIZE_8BIT, 
                     rxData, size, HAL_MAX_DELAY);
}
